...
So generators **don't** know about imports or file layout, they only produce named schema strings. `model.ts` owns assembly and returns `{ models, root }`: per-model files (written into the `models/` subdirectory) and root-level files (the shared `enums.ts` plus the `__nullable__` / `__transformDate__` helpers). `writer.ts` (`src/writer.ts`) formats each file with `oxfmt`, writes model files under `models/` and root files at the output root, and emits a root-level `model.ts` barrel that re-exports every model file (not enums or helpers). Because model files sit one directory deeper, their imports of the enums/helper files use `../`.
